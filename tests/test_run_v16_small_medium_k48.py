...
WRAPPER = ROOT / "scripts/run_v16_small_medium_k48_3090.sh"
DEFAULT_RUN_NAME = "candidate_selection_v16_mse5e-5_small_medium_sourcek20_kc48_p64_j64_linux_r1"
...
def _run(tmp_path, *options, wrapper=True, run_name="small_medium_k48_test"):
...
def _assert_mse_tolerance(output, expected, training_phase="train_fresh"):
    for phase in (
        training_phase, "inspect_checkpoint", "benchmark_six_methods_plus_verified",
...
def _assert_evaluation_scope(output):
    benchmark = _command(output, "benchmark_six_methods_plus_verified")
    ...
    for phase in ("benchmark_six_methods_plus_verified", "visualize_six_method_real_cases"):
        command = _command(output, phase)
        for option in ("--max-internal-knots", "--paper-initial-knots", "--liang-dense-knots"):
            assert command.count(option) == 1
            assert command[command.index(option) + 1] == "48"
        assert "--force-diagnostic" in command
        assert "--allow-unqualified-diagnostic" in command
    plot = _command(output, "plot_four_metrics")
    assert "--allow-unqualified-diagnostic" in plot
    assert "[paired_native_deployment]" not in output


def test_k48_wrapper_dry_run_passes_actual_training_parser_and_preserves_scope(tmp_path):
    result = _run(tmp_path)
    assert result.returncode == 0, result.stdout + result.stderr
    assert "SMALL_MEDIUM_K48 profile" in result.stdout
    ...
    assert "48 internal candidates, full cubic knot vector=56; baseline cap=48" in result.stdout
    ...
    assert args.study_scope == "small_medium_k48"
    assert (args.min_control_points, args.max_control_points, args.candidate_knots) == (8, 24, 48)
    ...
    _assert_evaluation_scope(result.stdout)


def test_k48_default_run_name_separates_all_new_artifacts_from_k24(tmp_path):
    result = _run(tmp_path, run_name=None)
    assert result.returncode == 0, result.stdout + result.stderr
    assert f"/checkpoints/{DEFAULT_RUN_NAME}.pt" in result.stdout
    for directory in ("teachers", "comparisons", "figures", "logs"):
        assert f"/{directory}/{DEFAULT_RUN_NAME}" in result.stdout
    assert "sourcek20_kc24" not in result.stdout
...
def test_k48_explicit_mse_tolerance_overrides_profile_in_either_argument_order(
    tmp_path, tolerance, profile_first,
):
    options = ["--mse-tolerance", tolerance]
    options.insert(0 if profile_first else len(options), "--small-medium-k48")
...
def test_k48_rejects_invalid_mse_tolerance_before_creating_artifacts(tmp_path, tolerance):
...
@pytest.mark.parametrize("profile_first", [False, True])
def test_k48_explicit_schedule_overrides_profile_in_either_argument_order(tmp_path, profile_first):
    options = ["--epochs", "80", "--proposal-epochs", "32", "--selector-warmup-epochs", "6"]
    options.insert(0 if profile_first else len(options), "--small-medium-k48")
    result = _run(tmp_path, *options, wrapper=False)
    assert result.returncode == 0, result.stdout + result.stderr
    args = _training(_command(result.stdout, "train_fresh"))
...
def test_k48_profile_allows_larger_full_benchmark_but_never_formal(tmp_path):
...
def test_k48_rejects_external_initializers_before_attempting_load(tmp_path, option):
    result = _run(tmp_path, option, "old_k24.pt")
    assert result.returncode != 0
    assert "--small-medium-k48 rejects" in result.stderr
    assert "[train_fresh]" not in result.stdout
    assert not (tmp_path / "outputs").exists()
...
def test_k48_rejects_reintroduction_of_high_k_training_or_validation(tmp_path, option, value):
    result = _run(tmp_path, option, value)
    assert result.returncode != 0
    assert f"--small-medium-k48 requires {option} 0" in result.stderr
    assert not (tmp_path / "outputs").exists()


@pytest.mark.parametrize("other", [
    "--pilot-kc56", "--keep-swap-highk72", "--keep-state-recovery", "--small-medium-k24",
])
@pytest.mark.parametrize("profile_first", [False, True])
def test_k48_rejects_all_conflicting_profiles_in_either_argument_order(tmp_path, other, profile_first):
    options = [other]
    options.insert(0 if profile_first else len(options), "--small-medium-k48")
    result = _run(tmp_path, *options, wrapper=False)
    assert result.returncode != 0
...
def test_k48_resume_keeps_this_run_and_zero_high_k_overrides(tmp_path):
    checkpoint_dir = tmp_path / "outputs/checkpoints"
    checkpoint_dir.mkdir(parents=True)
    for suffix in (".last.pt", ".proposal.pt"):
        (checkpoint_dir / f"small_medium_k48_test{suffix}").write_bytes(b"dry-run placeholder")
    result = _run(
    ...
    assert args.resume.as_posix().endswith("small_medium_k48_test.last.pt")
    assert args.output.as_posix().endswith("small_medium_k48_test.pt")
    assert args.study_scope == "small_medium_k48"
    assert (args.min_control_points, args.max_control_points, args.candidate_knots) == (8, 24, 48)
    ...
    assert (args.epochs, args.proposal_epochs, args.selector_warmup_epochs) == (128, 64, 4)
    assert args.mse_tolerance == pytest.approx(5e-5)
    _assert_mse_tolerance(result.stdout, 5e-5, training_phase="train_resume")
    _assert_evaluation_scope(result.stdout)


def test_k48_wrapper_is_thin_and_valid_bash():
    wrapper_source = WRAPPER.read_text(encoding="utf-8")
    assert 'exec bash "$SCRIPT_DIR/run_v16_mse1e-4_3090.sh" --small-medium-k48 "$@"' in wrapper_source
    for script in (SCRIPT, WRAPPER):
        syntax = subprocess.run([_bash(), "-n", str(script)], capture_output=True, text=True, check=False)
        assert syntax.returncode == 0, syntax.stderr


@pytest.mark.parametrize("profile,scope,candidates,epochs,warmup,tolerance", [
    (None, "legacy", 72, 128, 8, 1e-4),
    ("--small-medium-k24", "small_medium_k24", 24, 128, 4, 5e-5),
])
def test_k48_addition_preserves_default_and_previous_k24_profile(
    tmp_path, profile, scope, candidates, epochs, warmup, tolerance,
):
    result = _run(tmp_path, *([profile] if profile else []), wrapper=False)
    assert result.returncode == 0, result.stdout + result.stderr
    args = _training(_command(result.stdout, "train_fresh"))
    assert args.study_scope == scope
    assert args.candidate_knots == candidates
    assert (args.epochs, args.proposal_epochs, args.selector_warmup_epochs) == (epochs, 64, warmup)
    assert args.max_control_points == (24 if profile else 60)
    assert args.initial_keep_fraction == pytest.approx(0.5 if profile else 30 / 72)
    _assert_mse_tolerance(result.stdout, tolerance)
    benchmark = _command(result.stdout, "benchmark_six_methods_plus_verified")
    assert benchmark[benchmark.index("--max-knot-count") + 1] == ("20" if profile else "56")
    for option in ("--max-internal-knots", "--paper-initial-knots", "--liang-dense-knots"):
        assert benchmark[benchmark.index(option) + 1] == ("24" if profile else "56")
