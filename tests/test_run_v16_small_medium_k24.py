...
def _run(tmp_path, *options, wrapper=True):
    return subprocess.run(
        [
            _bash(), str(WRAPPER if wrapper else SCRIPT),
            "--dry-run", "--device", "cpu", "--run-name", "small_medium_test",
...
def test_k24_wrapper_dry_run_passes_actual_training_parser_and_preserves_scope(tmp_path):
    ...
    assert (args.epochs, args.proposal_epochs, args.selector_warmup_epochs) == (48, 24, 4)
