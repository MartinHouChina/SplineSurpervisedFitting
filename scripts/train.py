...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Train interactive structure-order prediction and dynamic "
            "single-count ordered knot decoding."
        )
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Generate a deterministic fresh synthetic training population each epoch.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Final probability of using the true knot count for position decoding.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "outputs" / "interactive_dynamic_v6.pt",
    )
    ...
    if (
        args.structure_attention_heads <= 0
        or args.hidden_dim % args.structure_attention_heads
    ):
        parser.error("--structure-attention-heads must divide --hidden-dim")
    ...
    val_set = SyntheticCubicBSplineDataset(
        size=args.val_size, seed=args.val_seed, **dataset_config
    )
    ...
    def teacher_forcing_schedule(epoch: int) -> float:
        if epoch < args.teacher_forcing_warmup_epochs:
            return 1.0
        anneal_steps = max(
            args.epochs - args.teacher_forcing_warmup_epochs - 1,
            1,
        )
        progress = min(
            (epoch - args.teacher_forcing_warmup_epochs) / anneal_steps,
            1.0,
        )
        return 1.0 + progress * (args.teacher_forcing_final - 1.0)
    ...
    checkpoint["selected_stage"] = "interactive_dynamic"
    checkpoint["stage_histories"] = {"interactive_dynamic": history}
