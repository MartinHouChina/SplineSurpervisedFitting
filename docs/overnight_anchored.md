...
2026-09-21：本页的新模型已在3090训练并拉回，结果见[配对审计](overnight_results_audit_20260921.md)。最新默认实验是[M16/M32两个通用模型](overnight_m16_m32.md)，不再按数据来源分别训练；增层与峰值误差说明见[Granularity](overnight_granularity.md)。以下保留当时的设计与本地验收记录，不把“尚未训练”的历史状态当作当前结果。
