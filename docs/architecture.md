...
当前新实验是 [中小 K 独立配置](v16_small_medium_k24.md)，`Kc=24`、合成 source K=4..20、MSE 阈值 `5e-5`，完整三次节点向量最多 32 项。它保留此前恢复档的交互实现，但从零训练，关闭高 K 采样，不加载 Kc56 完整权重。本次将 K24 阈值由 `1e-4` 收紧为 `5e-5`，训练由 24+24 调整为 Proposal 64 + Joint 64 = 128 代（Joint 已含 4 代 warmup），不改变下述网络结构。合成训练/验证仍为 600/160、每个真实来源验证最多 20、batch 32。此前 Kc56 [本地短训验证](v16_local_self_validation.md)尚未通过简化能力检查；缩小范围和接口测试不能替代新档的性能验证。旧检查点的新增开关默认关闭；下文显式状态交互及全局间隔调整由 `--small-medium-k24` 或历史 `--keep-state-recovery` 启用。
...
2. 选出 dense 验证较好的 Proposal，冻结编码器、参数头和候选头（含全局间隔调整）。在固定训练曲线上离线搜索满足本次 MSE 阈值（当前 K24 为 `5e-5`）的候选子集，缓存 KeepMask、数量、节点位置、删除风险及局部交换标签。
...
离线缓存绑定训练样本、Proposal 权重、间隔调整配置与阈值。缓存生成后不能继续修改 Proposal 而复用原标签。当前 `5e-5` 实验使用新的唯一运行名，重新生成对应认证数据和 Teacher 缓存，不得用旧 `1e-4` checkpoint 或缓存直接续跑。共享脚本用 `--mse-tolerance` 统一传递阈值，历史档未覆盖时仍默认 `1e-4`。原 `online_teacher` 和 `synthetic_ground_truth` 路径仍可用于历史对照，但不等于恢复档的离线教师协议。
