...
> 下文张量流是当前网络的原始一次性前向；**训练标签的来源已改变**。Joint 的 KeepMask/K 不再直接等于源真节点匹配，而由冻结 Proposal 上的离线可行子集教师给出。参见 [当前 v16 训练与部署工作流](v16_feasible_teacher_workflow.md)。部署后 MSE 核验/修复属于单独方法，不在本网络前向图中。
