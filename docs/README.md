...
最新实测：[本地短训、筛选与重定位验证](v16_local_self_validation.md)。Keep 状态恢复档目前尚未证明有效简化，不建议跳过该报告直接长训。

代码联调：[K24 检查与三项流程修复记录](v16_k24_code_audit.md)，涵盖四指标图读取、数值失败统计和分阶段续跑；流程修复不等于模型质量已经达标。

当前新实验为 [中小 K 独立配置](v16_small_medium_k24.md)：源内部 K=4..20、Kc=24、完整节点向量最多 32 项，`MSE<=1e-4`，从零训练 Proposal 24 + Joint 24 代。保留离线可行 Teacher 和 Keep 状态交互，不再混合高 K 训练；六方法统一内部节点预算 24。已检查接口与短流程，尚未证明新范围内的最终简化能力。
