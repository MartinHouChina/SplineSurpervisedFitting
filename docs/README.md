...
历史 Kc56 实测：[本地短训、筛选与重定位验证](v16_local_self_validation.md)。此前 Keep 状态恢复档尚未证明有效简化；该报告用于失败定位，不代表当前 K24、`5e-5`、128 代配置的实训结果。

代码联调：[K24 检查与三项流程修复记录](v16_k24_code_audit.md)，涵盖四指标图读取、数值失败统计和分阶段续跑；流程修复不等于模型质量已经达标。

当前新实验为 [中小 K 独立配置](v16_small_medium_k24.md)：源内部 K=4..20、Kc=24、完整节点向量最多 32 项，`MSE<=5e-5`，从零训练 Proposal 64 + Joint 64 = 128 代（Joint 已含 4 代 warmup）；合成训练/验证 600/160、每个真实来源验证最多 20、batch 32。保留离线可行 Teacher 和 Keep 状态交互，不再混合高 K 训练；六方法统一内部节点预算 24。旧 `1e-4`/48 代配置的接口与短流程检查保留为历史记录，不代表本次更严格阈值、128 代配置已完成实训或证明最终简化能力。

K24 默认运行名为 `candidate_selection_v16_mse5e-5_small_medium_sourcek20_kc24_p64_j64_linux_r1`；推荐确认名称未被使用，并显式传入 `--mse-tolerance 5e-5 --epochs 128 --proposal-epochs 64`。更换阈值须新建实验，不得静默复用旧阈值数据/Teacher 缓存或 checkpoint；历史共享入口默认阈值仍为 `1e-4`。
...
先阅读 [中小 K 范围、训练和 3090 命令](v16_small_medium_k24.md)，再查阅 [此前恢复实验的实测失败定位](v16_local_self_validation.md)及以下资料。
...
当前 Linux 一条龙入口为 [`run_v16_small_medium_k24_3090.sh`](../scripts/run_v16_small_medium_k24_3090.sh)，它调用共享脚本并启用独立 Kc24 档。输出 checkpoint、教师缓存、六方法及单列数值修复的测试记录、原六方法四指标图和外部案例图。该新范围按诊断协议运行，不能套用旧 K=4..56 的正式资格；文档说明协议，不代表新 checkpoint 已经充分训练或达标。
