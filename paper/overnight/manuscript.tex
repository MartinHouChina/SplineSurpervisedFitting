...
\end{equation}
$K$ counts interior knots only: $K=32$ means 40 knot-vector entries and
36 control points. Endpoint-constrained least squares computes
...
per epoch, and 500 synthetic validation curves. The source minimality audit
...
\item unified $K_c=32$ versus morphology-specific capacities chosen on
validation, without post-hoc selection by test error;
\item peak-loss on/off at the same MSE tolerance;
\item teacher geometry, redecoding verification, and parameter coupling.
\end{enumerate}
Specialists default to morphology-specific synthetic training with real
validation/testing only. Explicit real-training-split adaptation is a
different protocol and must be labelled separately. Test labels and test-fit
outcomes cannot choose capacity or model.
